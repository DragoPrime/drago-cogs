...
class HedoneRecommendation(commands.Cog):
    ...
    async def hedseturl(self, ctx, url: str):
    ...
    @commands.command()
    @commands.admin_or_permissions(administrator=True)
    async def hedsetapi(self, ctx, api_key: str):
    ...
    @commands.command()
    @commands.admin_or_permissions(administrator=True)
    async def hedsettmdbapi(self, ctx, api_key: str):
    ...
    @commands.command()
    @commands.admin_or_permissions(administrator=True)
    async def sethedchannel(self, ctx, channel: discord.TextChannel):
    ...
    @commands.command()
    @commands.admin_or_permissions(administrator=True)
    async def showhedsettings(self, ctx):
    ...
    async def recomandah(self, ctx):
        ...
        if not all(k in settings and settings[k] for k in ['base_url', 'api_key']):
            help_msg = (
                ...
                f"`{ctx.prefix}hedseturl <URL>` - Setează URL-ul serverului Jellyfin\n"
                f"`{ctx.prefix}hedsetapi <API_KEY>` - Setează cheia API Jellyfin\n"
                f"`{ctx.prefix}hedsettmdbapi <API_KEY>` - Setează cheia API TMDb pentru postere și descrieri (opțional)\n"
                f"`{ctx.prefix}sethedchannel <#CANAL>` - Setează canalul pentru recomandări\n\n"
                f"Poți verifica setările curente folosind `{ctx.prefix}showhedsettings`"
            )
